...
signing {
    ...
    if (isCI) {
        ...
        logger.lifecyle("isCI = $isCI")
        logger.lifecyle("signingKeyId = ${if (signingKeyId.isNullOrEmpty()) "NULL/EMPTY" else "SET"}")
        logger.lifecyle("signingKey = ${if (signingKey.isNullOrEmpty()) "NULL/EMPTY" else "SET (${signingKey?.length} chars)"}")
        logger.lifecyle("signingPassword = ${if (signingPassword.isNullOrEmpty()) "NULL/EMPTY" else "SET"}")

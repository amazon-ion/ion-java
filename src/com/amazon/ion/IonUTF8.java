...
public class IonUTF8 {
    ...
    public final static int getAs4BytesReversed(int unicodeScalar, byte[] outputBytes, int offset, int maxLength) 
    {
        int dst = offset;
    ...
    public final static int getScalarFrom4BytesReversed(byte[] bytes, int offset, int maxLength) 
    {
        int src = offset;
        int end = offset + maxLength;
        ...
        case 1:
            break;
        case 2:
            if (src+1 >= end) throw new ArrayIndexOutOfBoundsException();
            c  = (c & UNICODE_TWO_BYTE_MASK);
            c |= ((bytes[src++] & 0xff) & UNICODE_CONTINUATION_BYTE_MASK);
            break; 
        case 3:
            if (src+2 >= end) throw new ArrayIndexOutOfBoundsException();
            c  = (c & UNICODE_THREE_BYTE_MASK);
            c |= ((bytes[src++] & 0xff) & UNICODE_CONTINUATION_BYTE_MASK);
            c |= ((bytes[src++] & 0xff) & UNICODE_CONTINUATION_BYTE_MASK);
            break; 
        case 4:
            if (src+3 >= end) throw new ArrayIndexOutOfBoundsException();
